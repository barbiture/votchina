...
function cuSel(a) {
	...
	if (typeof(a['jScrollPane_borderTop']) === 'undefined') {
		a['jScrollPane_borderTop'] = 0;
	}
	if (typeof(a['jScrollPane_borderBottom']) === 'undefined') {
		a['jScrollPane_borderBottom'] = 0;
	}

	a['index'] = 0;
	jQuery('.cusel').each(function() {
		if (parseInt(jQuery(this).data('index'), 10) > parseInt(a['index'], 10)) {
			a['index'] = parseInt(jQuery(this).data('index'), 10);
		}
	});
	...
	jQuery(a.changedEl).each(function(c) {
		var d = jQuery(this),
			e = d.outerWidth(),
			f = d.prop("class"),
			cuSel_index = a['index'] + c + 1,
			g = d.prop("id") ? d.prop("id") : "cuSel-" + cuSel_index,
			h = d.prop("name"),
			j = d.val(),
		...
		var w = '<div id="cuselFrame-' + g + '" class="cusel ' + f + classDisCusel + '" style="width:' + e + 'px" tabindex="' + p + '">' + '<div class="cuselFrameRight"></div>' + '<div class="cuselText">' + l + '</div>' + '<div class="cusel-scroll-wrap"><div id="cusel-scroll-' + g + '" class="cusel-scroll-pane">' + u + '</div></div>' + '<input id="' + g + '" type="hidden" name="' + h + '" value="' + j + '" />' + '</div>';
		d.replaceWith(w);
		...
		if(y.length > a.visRows) {
			x.find(".cusel-scroll-wrap").eq(0).css({
				height: z * a.visRows + a.jScrollPane_borderTop + a.jScrollPane_borderBottom + "px",
				display: "none",
				visibility: "visible"
